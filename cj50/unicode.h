...
DEFTYPE_Result_(Option_(u32));


static
Result_(Option_(u32)) get_unicodechar(BufferedStream *in) {
    // https://en.wikipedia.org/wiki/Utf-8#Encoding
#define EBUFSIZ 256
    u32 codepoint;
    Result_(Option_(u8)) b1 = BufferedStream_getc(in);
    PROPAGATE_return(Option_(u32), b1);
    if (b1.ok.is_none) {
        return Ok(Option_(u32), None(u32));
    }
    if ((b1.ok.value & 128) == 0) {
        ...
        if        ((b1.ok.value & 0b11100000) == 0b11000000) {
            ...
            return Err(Option_(u32), literal_String(
                             "invalid start byte decoding UTF-8"));
        }
        for (int i = 1; i < numbytes; i++) {
            Result_(Option_(u8)) b = BufferedStream_getc(in);
            PROPAGATE_return(Option_(u32), b);
            if (b.ok.is_none) {
                char msg[EBUFSIZ];
                snprintf(msg, EBUFSIZ,
                         "premature EOF decoding UTF-8 (byte #%i)",
                         i+1);
                return Err(Option_(u32), copy_String(msg));
            }
    ...
    if (codepoint <= 0x10FFFF) {
        return Ok(Option_(u32), Some(u32, codepoint));
    } else {
        char msg[EBUFSIZ];
        snprintf(msg, EBUFSIZ,
